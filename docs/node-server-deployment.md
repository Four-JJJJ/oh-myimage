...
POST_PROCESSING_RETRY_ATTEMPTS=2
POST_PROCESSING_RETRY_DELAY_SECONDS=5
IMAGE_RETENTION_DAYS=90
INSPIRATION_FEATURE_ENABLED=false
```

该配置允许最多 10 个生图任务同时被 worker 处理。单任务最多 4 张图，`PROVIDER_IMAGE_CONCURRENCY=2` 会让 4 张图按 `2 + 2` 分批请求上游，因此真实上游图片请求峰值约为 20。Provider 请求默认不自动重发；后处理失败最多恢复同一份已接收结果 2 次，不会重新提交生图。
